...
class MessageDialog extends Component {
    ...
    static propTypes = {
         ...
         * the user.
         */
        doNotShowWarningLocalStorageKey: React.PropTypes.string,
    ...
    /**
     * Load from local storage the option does user had indicated that we should
     * not show this message dialog.
     *
     * @inheritdoc
     * @returns {void}
     */
    componentWillMount() {
        if (this.props.doNotShowWarningLocalStorageKey) {
            this.setState({
                doNotShowDialog:
                    jitsiLocalStorage.getItem(
                        this.props.doNotShowWarningLocalStorageKey)
                            === 'true'
            });
        } else {
            this.setState({
    ...
    _renderDoNotShowWarning() {
        if (!this.props.doNotShowWarningLocalStorageKey
            || !this.props.doNotShowWarningTextKey) {
            return null;
        }
    ...
    _onSubmit() {
        if (this.state.doNotShowWarningChecked) {
            jitsiLocalStorage.setItem(
                this.props.doNotShowWarningLocalStorageKey, 'true');
        }
